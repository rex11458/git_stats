...
class GitDataCollector(DataCollector):
	def collect(self, dir):
		...
		for line in lines:
			...
			if len(output) > 0:
				...
				try:
					...
					stamp = 0
				self.tags[tag] = { 'stamp': stamp, 'hash' : hash, 'date' : datetime.datetime.fromtimestamp(stamp).strftime('%Y-%m-%d'), 'commits': 0,'lines_added':0,'lines_removed':0, 'authors': {} }
		...
		for tag in reversed(tags_sorted_by_date_desc):
			cmd = 'git log --shortstat --date-order --pretty=format:"%%at %%aN" %s' % tag
			if prev != None:
				cmd += ' "^%s"' % prev
			prev = tag
					...
					if pos != -1:
						...
						try:
							...
							# clock skew, keep old timestamp to avoid having ugly graph
								stamp = oldstamp
							if author not in self.tags[tag]['authors']:
								self.tags[tag]['authors'][author] = { 'lines_added' : 0, 'lines_removed' : 0, 'commits' : 0}
							self.tags[tag]['authors'][author]['commits'] = self.tags[tag]['authors'][author].get('commits', 0) + 1
							self.tags[tag]['authors'][author]['lines_added'] =self.tags[tag]['authors'][author].get('lines_added', 0) + inserted
							self.tags[tag]['authors'][author]['lines_removed'] = self.tags[tag]['authors'][author].get('lines_removed', 0) + deleted

							self.tags[tag]['commits'] += 1
							self.tags[tag]['lines_added'] += inserted
							self.tags[tag]['lines_removed'] += deleted
		...
		# 		self.tags[tag]['authors'][author]['commits'] = commits
...
class HTMLReportCreator(ReportCreator):
	def create(self, data, path):
		...
		f.write('<tr><th>Name</th><th>Date</th><th>Commits</th><th>+lines</th><th>-lines</th><th>Authors</th></tr>')
		...
		for tag in tags_sorted_by_date_desc:
			...
			for i in reversed(self.authors_by_commits):
				authorinfo.append('%s (%d commits, %d +lines, %d -lines )' % (i, data.tags[tag]['authors'][i]['commits'],data.tags[tag]['authors'][i]['lines_added'],data.tags[tag]['authors'][i]['lines_removed']))
			f.write('<tr><td>%s</td><td>%s</td><td>%d</td><td>%d</td><td>%d</td><td>%s</td></tr>' % (tag, data.tags[tag]['date'], data.tags[tag]['commits'], data.tags[tag]['lines_added'],data.tags[tag]['lines_removed'], '<br />'.join(authorinfo)))
		f.write('</table>')
